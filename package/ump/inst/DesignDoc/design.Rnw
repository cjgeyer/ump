...
(this may require an infinite number of iterations of step~2 if the sample
space is infinite).
\end{enumerate}
...
In case the sample space is unbounded, $x \mapsto \phi(x, \alpha, \theta)$
given by \eqref{eq:umpu-crit} cannot be identically equal to zero,
which is required to satisfy \eqref{eq:umpu-a} and \eqref{eq:umpu-b}
when $\alpha = 0$.
But we do have $\phi(x, \alpha, \theta)$ converging pointwise to the constant
function $x \mapsto 0$ as $C_1$ approaches the lower boundary
(possibly $- \infty$)
and $C_2$ approaches the upper boundary (possibly $+ \infty$) and
$\gamma_1 = \gamma_2 = 0$.  But this does show that the algorithm given
above does find a solution for all $x$ and $0 < \alpha \le 1$ for each
$\theta$.
...
We need UMP one-tailed and UMPU two-tailed procedures for
\begin{itemize}
\item the binomial distribution (Sections~\ref{sec:model-binomial}
    and~\ref{sec:two-poisson-indep}
    and~\ref{sec:different-answers-same-question}
    and~\ref{sec:answers-different-questions}),
\item the Poisson distribution (Section~\ref{sec:model-poisson}),
\item the negative binomial distribution (Section~\ref{sec:negbin}),
\item the noncentral hypergeometric distribution
    (Section~\ref{sec:two-binom-indep} and~\ref{sec:exact}),
\item the exponential family generated by the negative hypergeometric
    distribution (Section~\ref{sec:two-negbin-indep}).
\end{itemize}
Thus we have five distributions (meaning families of distributions) to
work with.  Three are well-known distributions having good support in R.

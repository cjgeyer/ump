...
theory.  It is really different.  But \citet{geyer-meeden} do not use
...
described by membership functions that take values between
zero and one.
...
\item The function $1 - \phi(x, \alpha, \fatdot)$ they call the
    (membership function in the sense of fuzzy set theory of the)
...
the null when $P \le \alpha$ is the classical UMP or UMPU randomized test.
...
\item The UMP property
    ...
    other fuzzy confidence interval having the exactness and unbiasednessi
    properties.
\end{itemize}
...
bogus results or software crashes in unusual special cases.
...
support except the least (if there is a least) there is a next lower point
in the support,
and for every point $x$ of the support except the greatest
(if there is a greatest) there is
...
assume $T(X) = X$ so we have a standard exponential family, define
$j = 3 - i$, and define
\begin{align*}
...
\end{align*}
(either of these can be empty, but not both by the assumption the support
...
assumption that we have a regular full exponential family.
...
If the maximum of the membership function is less than one,
then the maximum occurs at $\mu = x$
...
hence from \eqref{eq:umpu-a} and \eqref{eq:umpu-b}
\begin{align*}
   E_\mu\{I_{(\mu, \infty)}(X) \phi(X, \alpha, \mu)\} & \to \alpha
   \\
   E_\mu\{X I_{(\mu, \infty)}(X) \phi(X, \alpha, \mu)\} / \mu & \to \alpha
\end{align*}
so
$$
   E_\mu\left\{\left(\frac{X}{\mu} - 1\right) I_{(\mu, \infty)}(X)
   \phi(X, \alpha, \mu)\right\} \to 0
$$
\end{proof}
...
\begin{itemize}
\item If the support of the canonical statistic has two points,
...
\section{Calculations For Distributions} \label{sec:models}
...
UMP or UMPU competitor of McNemar's test (Lindgren,
...
When we return values of the function at points not specified by the user,
we will also have to return those $\alpha$ values (for a fuzzy $P$-value)
or $\theta$ values (for a fuzzy confidence interval).
Thus we will return a list, and we might as well return a list specifying
everything: $\phi(x, \alpha, \theta)$, $x$, $\alpha$, $\theta$, and the
distribution and type of test (lower, upper, two-tailed), or, in the case
of wanting the PDF of the fuzzy $P$-value,
$\partial \phi(x, \alpha, \theta) / \partial \alpha$ rather than
$\phi(x, \alpha, \theta)$.

We need make no special allowance for fuzzy decision functions
$\phi(\fatdot, \alpha, \theta)$, since the usual R behavior will
accomodate that.
...
As seen in Algorithm~\ref{alg:lots-of-work} and various theorems, testing
whether $x = \mu$ or not is important.  We will not be able to do this with
exact arithmetic if $\mu$ is calculated rather than provided by the user.
Thus we should use $\mu$ as the parameter if at all possible.
\begin{itemize}
\item For the binomial distribution, we should use $\mu = n p$ as the
    parameter rather than $p$, which R uses for \code{dbinom} and friends.
    If we have to multiply $n$ times $p$, that will not be exact arithmetic,
    so the test may be in error.
\item For the Poisson distribution, $\mu$ is already the usual parameter.
\item For the negative binomial distribution, $\mu$ is already an optional
    parameter in \code{dnbinom} and friends.  And the C interface provided
    to R packages has C functions \verb@dnbinom_mu@, \verb@pnbinom_mu@,
    \verb@qnbinom_mu@, \verb@rnbinom_mu@ that use $\mu$ as a parameter.
\item For the noncentral hypergeometric distribution we have the problem
    that we do not have a formula expressing the mapping between canonical
    and mean value parameters.  So we may have to use the canonical parameter
    for it.  Exception: for $\theta = 0$ we know $\mu = n K / N$
    in the notation of Section~\ref{sec:noncentral-hypergeometric},
    that is, when we have regular hypergeometric rather than noncentral
    hypergeometric.  Since this is a widely used null hypothesis,
    we want to special case this.
\item Similarly, for the noncentral negative hypergeometric distribution we
    also must use the canonical parameter, except when $\theta = 0$ when
    we know $\mu = r K / (N - K + 1)$
    in the notation of Section~\ref{sec:noncentral-negative-hypergeometric}.
\end{itemize}

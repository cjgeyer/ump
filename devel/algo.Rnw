...
\item The function $1 - \phi(x, \alpha, \fatdot)$ they call
    ...
    fuzzy confidence interval with coverage $1 - \alpha$.
...
\item The \emph{exactness} property is $c(\theta, \theta) = 1 - \alpha$
    for all $\theta$.  We have the exact desired coverage, unlike classical
...
So everything depends on the critical function $\phi(x, \alpha, \theta)$ for
...
We will say that a random variable or its distribution
is \emph{topologically discrete} if the support, considered as a subspace
of the real line, has the discrete topology: for every point $x$ of the
support there is an $\varepsilon > 0$ such that there are no other
points of the support between $x - \varepsilon$ and $x + \varepsilon$.
This property
...
topologically discrete example.
A mixture of an atom at zero and $1 / X$ where $X$ is Poisson does the job.
There is no neighborhood of zero that does not contain other points of the
support.  But I do not know what an example
...
canonical statistic is probabilistically and topologically discrete.
For either UMP or UMPU
the function $F_x$ defined by $F_x(\alpha) = \phi(x, \alpha, \theta)$
is continuous, piecewise linear, nondecreasing on the interval $[0, 1]$,
and maps onto the interval $[0, 1]$.
$F_x$ is strictly increasing near $\alpha$ such that $0 < F_x(\alpha) < 1$.
Hence $F_x$ is the distribution function of a continuous
...
Assume a regular full one-parameter exponential family such that the
canonical statistic is topologically discrete.
...
Without the topologically discrete assumption, it could be
...
$F_x$ is continuous, piecewise linear, and increasing
...
Algorithm~\ref{alg:lots-of-work}, then we have $F_x(\alpha) = 0$
for the rest of the execution of the algorithm.  Thus the only way we can
have $F_x(\alpha) > 0$ for the whole execution is,
...
close to zero, and when it does $F_x(\alpha)$ also gets
...
statistic is probabilistically and topologically discrete is convex,
...
or either of the two highers.  So the fuzzy confidence interval has the
